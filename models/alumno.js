...
Alumno.delete = function(nc, resultado){
    conexion.query("DELETE FROM alumnos WHERE numControl = ?", [nc], function(err, resp){
        if(err){
            console.log("Error: ", err);
            resultado(null, err);
        }else{
...
Alumno.update = function(nc, alumno, resultado){
    conexion.query("UPDATE alumnos SET nombre=? WHERE numControl=?", [alumno.n, alumno.nc], function(err, resp){
        if(err){
            console.log("Error: ", err);
            resultado(null, err);
        }else{
